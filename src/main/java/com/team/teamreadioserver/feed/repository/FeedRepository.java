...
//              (SELECT COUNT(pl.likes_id) FROM post_likes pl WHERE pl.post_id = p.post_id) AS likesCount,
//              (SELECT COUNT(prv.post_review_id) FROM post_review prv WHERE prv.post_id = p.post_id) AS reviewsCount,
//              (SELECT CASE WHEN NULL IS NOT NULL AND EXISTS(SELECT 1 FROM post_likes pl WHERE pl.post_id = p.post_id AND pl.profile_id = NULL) THEN TRUE ELSE FALSE END) AS isLiked,
//              (SELECT CASE WHEN NULL IS NOT NULL AND EXISTS(SELECT 1 FROM follow f WHERE f.follower_profile_id = NULL AND f.following_profile_id = pr.profile_id) THEN TRUE ELSE FALSE END) AS isFollowing
//          FROM
//              post p
//          JOIN
//              profile pr ON p.profile_id = pr.profile_id
