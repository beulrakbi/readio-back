...
public class BookReviewService {
    ...
    public void addBookReview(ReviewRequestDTO reviewRequestDTO) {
        BookReview bookReview = BookReview.builder().bookIsbn(reviewRequestDTO.getBookIsbn()).reviewContent(reviewRequestDTO.getReviewContent()).reportedCount(0).build();
    ...
    public void reportReview(Integer reviewId) {
        BookReview bookReview = bookReviewRepository.findById(reviewId).orElseThrow(() -> new IllegalArgumentException("해당 리뷰가 존재하지 않습니다."));
        bookReview.report();
        if (bookReview.getReportedCount() == 1) {
            Profile profile = profileRepository.findByProfileId(bookReview.getProfileId());
    ...
    public List<AllReviewResponseDTO> allBookReview() {
        return bookReviewRepository.findAll().stream().map(review -> AllReviewResponseDTO.builder().profileId(review.getProfileId()).reviewContent(review.getReviewContent()).createdAt(review.getCreatedAt()).build()).toList();
    }


    //내 리뷰 (피드)
    public List<MyReviewResponseDTO> myBookReview(Integer profileId) {
        return bookReviewRepository.findByProfileId(profileId).stream().map(review -> MyReviewResponseDTO.builder().reviewContent(review.getReviewContent()).createdAt(review.getCreatedAt()).build()).toList();
    }
    ...
    public void addLikeBookReview(Integer reviewId, Integer profileId) {
        ...
        ReviewLike reviewLike = ReviewLike.builder().reviewId(reviewId).profileId(profileId).build();
